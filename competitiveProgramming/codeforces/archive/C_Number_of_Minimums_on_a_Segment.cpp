...
void solve() {
    ...
    auto small = [](int a, int b){return min(a,b);};
    segment_tree<ll,decltype(small)> st(n,INT32_MAX,small);
    //for(int i = 0; i < n; i++){ int temp; cin >> temp; st.assign(i,temp); }
    st.assign([](){
        int temp; cin >> temp;
        return temp;
    });
