...
# no padding: (32-3+1)//2 => 15; (15-3+1)//2 => 6; (5-3+1)//2 = 2 => 5x5 image; 16 channels
    ...
    def forward(self, x): # use all layers here and compute prediction
        x = F.max_pool2d( F.relu( self.conv1(x) ), 2, padding=1)
        x = F.max_pool2d( F.relu( self.conv2(x) ), 2, padding=1)
        x = F.max_pool2d( F.relu( self.conv3(x) ), 2, padding=1)
        x = F.max_pool2d( F.relu( self.conv4(x) ), 2, padding=1)
